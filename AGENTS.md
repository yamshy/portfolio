...
This repository contains a modern **Astro v5** portfolio site with a Svelte integration, written in **TypeScript** and managed with **pnpm**. Agents are used to automate routine development tasks—installing dependencies, running type checks, executing tests, building the site, and maintaining release workflows.

# Prerequisites

- **Node.js**: v24.7.0 via [`mise`](.mise.toml) (CI currently uses Node 20; any Node ≥20 works)
- **pnpm**: v10.15.0
...
| `check:astro`   | Astro static analysis                | `astro check`                                                                         |
| `check:types`   | TypeScript type checking             | `tsc --noEmit`                                                                        |
| `lint`          | ESLint static analysis               | `eslint .`                                                                            |
| `lint:fix`      | ESLint with autofix                  | `eslint . --fix`                                                                      |
| `format:check`  | Prettier formatting check            | `prettier -c .`                                                                       |
| `format:write`  | Prettier format in-place             | `prettier -w .`                                                                       |
| `check:all`     | Run all checks sequentially          | `pnpm check:astro && pnpm check:types && pnpm lint && pnpm format:check && pnpm test` |
...
- Default branch: `main`
- PR workflow `.github/workflows/pr-checks.yml` runs type check (`tsc --noEmit`), `astro check`, and `astro build`, verifies Conventional Commit titles, and performs a Docker smoke test【F:.github/workflows/pr-checks.yml†L1-L90】
- Release workflow builds and publishes a container image to GHCR using semantic-release【F:release.config.js†L1-L13】【F:.github/workflows/release.yml†L1-L60】
...
- `NODE_ENV` defaults to `development` in `.mise.toml`【F:.mise.toml†L5-L6】

# Agent Tasks & Safe Operations

- Agents may modify source, tests, docs, and CI workflows.
- Ensure all changes pass `astro check`, `tsc --noEmit`, tests, and formatting.
...
1. `pnpm install`
2. Run `pnpm astro check`, `pnpm check:types`, `pnpm lint`, `pnpm format:check`, and `pnpm test`
3. Fix any issues; commit using Conventional Commit messages.
4. Push branch and open PR.

### Refactor or Add a Feature

1. Update or add tests under `tests/`.
2. Ensure type safety and run full check suite.
...
- **Release**: semantic-release determines version, updates changelog, builds and pushes Docker image to GHCR.【F:release.config.js†L1-L13】
- Match CI locally by running the same commands before pushing.

# Troubleshooting

- `pnpm astro check` currently fails at `src/pages/index.astro` because `CTAButton` lacks a `class` prop; add it to the component's props to resolve the error【F:src/pages/index.astro†L147-L149】
